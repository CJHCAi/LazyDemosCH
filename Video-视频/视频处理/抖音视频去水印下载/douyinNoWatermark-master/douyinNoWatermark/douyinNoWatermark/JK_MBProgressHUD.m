...
#define JK_MBMainThreadAssert() NSAssert([NSThread isMainThread], @"JK_MBProgressHUD needs to be accessed on the main thread.");

CGFloat const JK_MBProgressMaxOffset = 1000000.f;

static const CGFloat JK_MBDefaultPadding = 4.f;
static const CGFloat JK_MBDefaultLabelFontSize = 16.f;
static const CGFloat JK_MBDefaultDetailsLabelFontSize = 12.f;


@interface JK_MBProgressHUD ()
...
@interface JK_MBProgressHUDRoundedButton : UIButton
@end


@implementation JK_MBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    JK_MBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    JK_MBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (JK_MBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            JK_MBProgressHUD *hud = (JK_MBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = JK_MBProgressHUDAnimationFade;
    _mode = JK_MBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(JK_MBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == JK_MBProgressHUDAnimationZoom) {
        type = animatingIn ? JK_MBProgressHUDAnimationZoomIn : JK_MBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == JK_MBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == JK_MBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == JK_MBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == JK_MBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    JK_MBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<JK_MBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    JK_MBBackgroundView *backgroundView = [[JK_MBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = JK_MBProgressHUDBackgroundStyleSolidColor;
    ...
    JK_MBBackgroundView *bezelView = [JK_MBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:JK_MBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:JK_MBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [JK_MBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:JK_MBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[JK_MBRoundProgressView class]];

    JK_MBProgressHUDMode mode = self.mode;
    if (mode == JK_MBProgressHUDModeIndeterminate) {
    ...
    else if (mode == JK_MBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[JK_MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == JK_MBProgressHUDModeDeterminate || mode == JK_MBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[JK_MBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == JK_MBProgressHUDModeAnnularDeterminate) {
            [(JK_MBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == JK_MBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == JK_MBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[JK_MBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[JK_MBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[JK_MBRoundProgressView class]]) {
        JK_MBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [JK_MBRoundProgressView appearanceWhenContainedIn:[JK_MBProgressHUD class], nil];
#else
        appearance = [JK_MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[JK_MBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((JK_MBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((JK_MBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[JK_MBBarProgressView class]]) {
        JK_MBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [JK_MBBarProgressView appearanceWhenContainedIn:[JK_MBProgressHUD class], nil];
#else
        appearance = [JK_MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[JK_MBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((JK_MBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((JK_MBBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    JK_MBBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? JK_MBDefaultPadding : 0.f;
...
- (void)setMode:(JK_MBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == JK_MBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation JK_MBRoundProgressView
...
@implementation JK_MBBarProgressView
...
@implementation JK_MBBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = JK_MBProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(JK_MBProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    JK_MBProgressHUDBackgroundStyle style = self.style;
    if (style == JK_MBProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == JK_MBProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation JK_MBProgressHUDRoundedButton
