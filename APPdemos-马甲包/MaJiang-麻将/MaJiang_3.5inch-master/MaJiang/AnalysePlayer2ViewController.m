...
#import "AnalysePlayer2ViewController.h"
#import "PNChart.h"
...
@interface AnalysePlayer2ViewController ()
{
...
@implementation AnalysePlayer2ViewController
...
- (void)viewDidLoad
{
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK)
    {
        ...
        for (int i=0; i<DBgameid; i++) {
            NSString *sqlQuery2 = [NSString stringWithFormat:@"SELECT SUM(PLAYER2) FROM POINTS WHERE GAMEID = %d", i + 1];
        ...
        if (sqlite3_prepare_v2(database, [sqlQuery3 UTF8String], -1, &statement, nil) == SQLITE_OK) {
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ...
                if ([nsfactor3 isEqualToString:@"下家"]) {
                    //NSLog(@"%@", nsfactor3);
                    if ([nsfactor2 isEqualToString:@"接炮"]) {
                        dianpao++;
                    }
