...
@interface TYTabPagerController ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
{
    ...
    struct {
        ...
        unsigned int canMoveItemAtIndexPath :1;
...
@property (nonatomic, strong, readwrite) UIView *pagerBarUnderLineView;
...
- (void)viewWillLayoutSubviews
{
    ...
    self.pagerBarUnderLineView.frame = CGRectMake(0, CGRectGetHeight(_pagerBarView.frame)-0.5f, CGRectGetWidth(self.view.frame), 0.5f);
...
- (void)addPagerBarView
{
    ...
    [_pagerBarView addSubview:self.pagerBarUnderLineView];
}
...
- (void)setDelegate:(id<TYTabPagerControllerDelegate>)delegate
{
    ...
    _tabDelegateFlags.canMoveItemAtIndexPath = [self.delegate respondsToSelector:@selector(pagerController:canMoveItemAtIndexPath:)];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_tabDataSourceFlags.titleForIndex) {
        id title = [self.dataSource pagerController:self titleForIndex:indexPath.item];
...
{
    if (_tabDelegateFlags.canMoveItemAtIndexPath) {
        if ([self.delegate pagerController:self canMoveItemAtIndexPath:indexPath]) {
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (_cellWidth > 0) {
        ...
        if ([title isKindOfClass:[NSString class]]) {
            width = [self boundingSizeWithString:title font:_selectedTextFont constrainedToSize:CGSizeMake(300, 100)].width+_cellEdging*2;
            if (self.collectionLayoutEdging == 0.f && self.cellSpacing == 0.f) {
                width += 40;
            }
        }
...
- (void)setIsShowUnderLineView:(BOOL)isShowUnderLineView {
    _isShowUnderLineView = isShowUnderLineView;
    self.pagerBarUnderLineView.hidden = !_isShowUnderLineView;
}

#pragma mark -

- (UIView *)pagerBarUnderLineView {
    if (!_pagerBarUnderLineView) {
        _pagerBarUnderLineView = [[UIView alloc] init];
//        _pagerBarUnderLineView.backgroundColor = [UIColor colorWithRGB:0xe2e1e1];
    }
    return _pagerBarUnderLineView;
}
