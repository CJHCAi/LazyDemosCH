...
- (void)segmentPageSetting {
    ...
    _contentView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0,kScreenWidth, _segmentHeight)];
   ...
   // _indicateView.backgroundColor = _segmentTintColor;
    _indicateView.backgroundColor =RGB(239,89,60);
    
    // CGFloat width =SCREEN_WIDTH_S /_titles.count;
    CGFloat item_x = 0;
    for (int i = 0; i < _titles.count; i++) {
      ...
      // button.frame = CGRectMake(i*width, 0,width, _segmentHeight);
...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:_duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (_style == ZWMSegmentStyleDefault) {
            _indicateView.frame = CGRectMake(index *_selectedButton.frame.size.width, CGRectGetMinY(_indicateView.frame),_selectedButton.frame.size.width, _indicateHeight);
...
- (void)scrollSegementView {
    ...
    if (_selectedButton.frame.origin.x <= _size.width / 2) {
        ...
        [_contentView setContentOffset:CGPointMake(_contentView.contentSize.width - _size.width, 0) animated:NO];
    } else {
        [_contentView setContentOffset:CGPointMake(CGRectGetMinX(_selectedButton.frame) - offsetX, 0) animated:NO];
    }
}
