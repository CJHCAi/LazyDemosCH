...
/** 长按手势 */
@property (nonatomic, strong) UILongPressGestureRecognizer *long_press_imageViewGesture;
...
-(void)awakeFromNib{
    ...
    [self addGestureRecognizer:self.long_press_imageViewGesture];
}
...
//            NSString *str=[NSString stringWithFormat:@"%ld",(long)expectedSize];
//            
//            [str stringByReplacingOccurrencesOfString:@"-" withString:@""];
            
            expectedSize = expectedSize<0 ? 1:expectedSize;//修复-0问题
            
            CGFloat progress = 1.0 *receivedSize /((CGFloat)expectedSize);
 ...
 * 长按手势
 */
- (void)long_press_imageViewTap:(UIGestureRecognizer *)tap{
    if(tap.state != UIGestureRecognizerStateBegan){
        return;
    }
    if(_ItemViewLongPressBlock != nil) _ItemViewLongPressBlock();
}
...
/**
 ...
 @return 长按手势
 */
- (UILongPressGestureRecognizer *)long_press_imageViewGesture{
    
    if(_long_press_imageViewGesture == nil){
        _long_press_imageViewGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(long_press_imageViewTap:)];
    }
    return _long_press_imageViewGesture;
}
