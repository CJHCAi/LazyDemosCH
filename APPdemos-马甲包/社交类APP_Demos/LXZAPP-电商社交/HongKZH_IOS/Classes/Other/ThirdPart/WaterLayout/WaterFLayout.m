...
//  XiYou_IOS
//
//  Created by reganan on 15/12/4.
//  Copyright © 2015年 reganan. All rights reserved.
//
...
- (void)prepareLayout
{
    ...
    if (self.isBeforeSubtraction) {
        self.itemWidth = floorf(((width - self.rightMark) - (self.columnCount - 1) * self.minimumColumnSpacing) / self.columnCount);
    }else{
    self.itemWidth = floorf((width - (self.columnCount - 1) * self.minimumColumnSpacing) / self.columnCount)-self.rightMark;
    }
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section)
    {
        ...
        for (idx = 0; idx < itemCount; idx++)
        {
            ...
            NSUInteger columnIndex = [self shortestColumnIndex];    //最短的列
            ...
            {
                attributes.frame = CGRectMake(xOffset-0.25, yOffset, self.itemWidth, itemHeight);
            }
