...
@property (strong, nonatomic) UIView *currentPosView;
@property (strong, nonatomic) TRSDialView *dialView;
@property (assign, nonatomic) BOOL isHorizontal;    //NO:Vertial, YES:Horizontal
@property (assign, nonatomic) NSInteger offset;     //Offset of currentPos
...
- (void)commonInit
{
    ...
    if(_isHorizontal)
    {
        ...
        _currentPosView = [[UIView alloc] initWithFrame:CGRectMake(self.bounds.size.width / 2, 0, 1, contentHeight / 2)];
    }
    else
    {
        _dialView = [[TRSDialView alloc] initWithFrame:CGRectMake(0, 0, contentWidth, self.bounds.size.height)];
        _currentPosView = [[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height / 2, contentWidth / 2, 1)];
    }
    ...
    _currentPosView.backgroundColor = [UIColor redColor];
    ...
    [self addSubview:_currentPosView];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _isHorizontal = YES;
        [self commonInit];
        _dialView.isHorizontal = _isHorizontal;
    }
...
- (void)setDirection:(BOOL)isHorizontal
{
    ...
    if(_isHorizontal)
    {
        _dialView.frame = CGRectMake(0, 0, self.bounds.size.width, contentHeight);
        _currentPosView.frame = CGRectMake(self.bounds.size.width / 2, 0, 1, contentHeight / 2);
        _scrollView.contentSize = CGSizeMake(_dialView.frame.size.width, contentHeight);
    }
    else
    {
        _dialView.frame = CGRectMake(0, 0, contentWidth, self.bounds.size.height);
        _currentPosView.frame = CGRectMake(0, self.bounds.size.height / 2, contentWidth / 2, 1);
        _scrollView.contentSize = CGSizeMake(contentWidth, _dialView.frame.size.height);
    }
}

- (void)setOffset:(NSInteger)nOffset
{
    _offset = nOffset;
    _dialView.offset = nOffset;

    float contentHeight = self.bounds.size.height;
    float contentWidth = self.bounds.size.width;
    if(_isHorizontal)
    {
        _currentPosView.frame = CGRectMake(self.bounds.size.width / 2 + self.dialView.minorTickDistance * _offset, 0, 1, contentHeight / 2);
    }
    else
    {
        _currentPosView.frame = CGRectMake(0, self.bounds.size.height / 2 - self.dialView.minorTickDistance * _offset, contentWidth / 2, 1);
    }
}
...
- (void)setCurValueViewLength:(NSInteger)curValueViewLength
{
    _currentPosView.frame = CGRectMake(self.bounds.size.width / 2, 0, 1, curValueViewLength);
}
...
-(void)setDataFormat:(DATA_FORMAT_TYPE)dataFormat
{
    self.dialView.dataFormat = dataFormat;
}
