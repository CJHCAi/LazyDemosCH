...
- (void)setUpFrames{
    ...
    _delButton.frame = CGRectMake(kAPPWidth - 55 - 30, kAppHeight - 70, 55, 30);
}
...
- (void)addToolbars{
    ...
    delButton.layer.borderWidth = 0.1;
    delButton.layer.borderColor = [UIColor whiteColor].CGColor;
    delButton.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.3f];
    delButton.layer.cornerRadius = 2;
...
- (void)deleteImage{
    if (self.delegate) {
        [self.delegate deletePhoto:self at:self.currentImageIndex];
    }
}
...
- (void)hidePhotoBrowser:(UITapGestureRecognizer *)recognizer{
    ...
    [self hideBrowerFrom:view];
}

- (void)hideBrowerFrom:(HZPhotoBrowserView *)view{
