...
- (void)layoutSubviews {
    ...
    self.customImageView.frame = CGRectMake(self.iconLeftMargin, (self.frame.size.height - separaterHeight - self.iconSize.height) * 0.5+5 ,self.iconSize.width-10, self.iconSize.height-10);
    //标题
    CGFloat labelX = CGRectGetMaxX(self.customImageView.frame) + self.iconRightMargin;
    
    self.customTitleLabel.frame = CGRectMake(labelX, 0, self.frame.size.width, self.frame.size.height - separaterHeight);
