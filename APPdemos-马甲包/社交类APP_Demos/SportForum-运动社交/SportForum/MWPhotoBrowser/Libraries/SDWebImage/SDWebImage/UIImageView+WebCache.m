...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder withInset:(CGFloat) inset
{
    [self sd_setImageWithURL:url placeholderImage:placeholder withInset:inset options:0 progress:nil completed:nil];
}
...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder withInset: (CGFloat) inset options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock;
{
    [self sd_cancelCurrentImageLoad];
    objc_setAssociatedObject(self, &imageURLKey, url, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    if (!(options & SDWebImageDelayPlaceholder)) {
        placeholder = [self ellipseImage:placeholder withInset:0];
    ...
    if (url) {
        ...
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:options progress:progressBlock completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            ...
            dispatch_main_sync_safe(^{
                if (!wself) return;
                UIImage *imagNew = [wself ellipseImage:image withInset:inset];
                ...
                if (completedBlock && finished) {
                    completedBlock(imagNew, error, cacheType, url);
                }
            });
        }];
...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock {
    if ([[url description] isEqualToString:[self.sd_imageURL description]]) {
        return;
    }
