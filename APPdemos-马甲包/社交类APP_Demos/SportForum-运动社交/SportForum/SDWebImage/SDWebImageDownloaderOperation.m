...
- (void)start
{
    ...
    __weak SDWebImageDownloaderOperation* thisPointer = self;
    dispatch_async(dispatch_get_main_queue(), ^
    {
        if (self.isCancelled)
        {
            thisPointer.finished = YES;
            [thisPointer reset];
            return;
        }
        ...
        if (!(thisPointer.options & SDWebImageDownloaderLowPriority))
        {
            [thisPointer.connection scheduleInRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
        }

        [self.connection start];

        if (thisPointer.connection)
        {
            if (thisPointer.progressBlock)
            {
                thisPointer.progressBlock(0, -1);
            }
            [[NSNotificationCenter defaultCenter] postNotificationName:SDWebImageDownloadStartNotification object:self];
        }
        else
        {
            if (thisPointer.completedBlock)
            {
                thisPointer.completedBlock(nil, nil, [NSError errorWithDomain:NSURLErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey: @"Connection can't be initialized"}], YES);
            }
        }
    });
}
