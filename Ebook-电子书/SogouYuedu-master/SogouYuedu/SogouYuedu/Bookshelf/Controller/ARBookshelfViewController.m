...
#import "ARBookshelfViewController.h"
...
#import "ARRegularExpression.h"
#import "ARChaptersItem.h"
#import "ARNovelContentController.h"
#import "ARDetailNobkeyViewController.h"

@interface ARBookshelfViewController ()<SelectIndexPathDelegate,UIGestureRecognizerDelegate,UICollectionViewDelegateFlowLayout>
{
...
@implementation ARBookshelfViewController
...
- (void)viewWillAppear:(BOOL)animated{
    booksArr = [ARBookItem readDatabase];
    //添加最后一项，是一个带加号的图片
    ARBookItem *itm = [[ARBookItem alloc]init];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ARBookCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseIdentifier forIndexPath:indexPath];
    
    ARBookItem *item = booksArr[indexPath.row];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == booksArr.count -1 ) {
        ...
        ARBookItem *bookitem = booksArr[indexPath.row];
        
        //2.根据name拼接表名，便于查询章节数据表
        NSString *tableStr = [NSString stringWithFormat:@"t_%@chapters",bookitem.name];
        //目前只读取第一章
        NSInteger pageIndex = 1;
        NSMutableArray *chaptersArray = [NSMutableArray arrayWithArray: [ARChaptersItem readOneChapterFromTable:tableStr Index:pageIndex]];
        ARChaptersItem *chapterItem = [[ARChaptersItem alloc]init];
        for (ARChaptersItem *chapter in chaptersArray) {
            chapterItem = chapter;
        }
        ...
        if (bookitem.bkey.length > 8) { //没有bkey的，值为"(null)" 所以长度至少要大于6 才能判断
            ...
            ARNovelContentController *NCVC = [[self storyboard]instantiateViewControllerWithIdentifier:@"NovelContentVC"];
            if (txt) {
                //如果有文件，则直接推送到阅读器打开
                [NCVC loadText:txt];
...
- (void)downloadZip:(NSString *)urlStr{
    ...
    [[[NSURLSession sharedSession] downloadTaskWithURL:url completionHandler:^(NSURL *location, NSURLResponse *response, NSError *error) {
        ...
        ARNovelContentController *NCV = [[self storyboard]instantiateViewControllerWithIdentifier:@"NovelContentVC"];
...
- (void)NobkeyChapterContentRequest:(NSString *)urlStr{
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        NSString * realContent = [ARRegularExpression getChapter:str pattern:@"(\\{).*?\\}.*?(\\})"];
        //跳转到阅读器界面,必须在主线程里执行
        dispatch_async(dispatch_get_main_queue(), ^{
