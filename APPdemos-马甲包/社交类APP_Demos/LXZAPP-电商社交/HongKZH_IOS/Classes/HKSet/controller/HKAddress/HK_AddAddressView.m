...
@interface HK_AddAddressView ()<UITextFieldDelegate,UIPickerViewDelegate ,UIPickerViewDataSource,UIActionSheetDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate,VPImageCropperDelegate,UITableViewDataSource,UITableViewDelegate>
{
    NSInteger row1;
    NSInteger row2;
    NSInteger row3;
...
@implementation HK_AddAddressView
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    [rightB1Two1 setBackgroundImage:[UIImage imageNamed:@"limit_red"] forState:UIControlStateSelected];
...
{
    _rightB1Two1.selected =!_rightB1Two1.selected;
}
...
#pragma mark 根据是否选择默认
    ...
    if (self.isRefund) {
        [HKAfterSaleViewModel addUserReturnAddress:dic type:YES  success:^(HKBaseResponeModel *responde) {
            if (responde.responeSuc) {
                if ([self.delegate respondsToSelector:@selector(refreshLastVc)]) {
                    [self.delegate refreshLastVc];
                }
                [self.navigationController popViewControllerAnimated:YES];
            }
        }];
