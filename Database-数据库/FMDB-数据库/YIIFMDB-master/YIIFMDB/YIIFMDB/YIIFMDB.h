 ...
 *  @param parameters   参数，YIIParameters决定了sql语句"where"后面的参数。具体用法参考YIIParameters类
 */
- (BOOL)updateTable:(NSString * _Nullable)tableName dictionary:(NSDictionary * _Nonnull)dictionary whereParameters:(YIIParameters *_Nullable)parameters;
 ...
 *  @param parameters   参数，YIIParameters决定了sql语句"where"后面的参数。具体用法_Nullable参考YIIParameters类
 *  @return             返回所有符合条件的数据
 */
- (NSArray *_Nullable)queryFromTabl_Nullablee:(NSString *_Nullable)tableName model:(Class _Nullable )modelClass whereParameters:(YIIParameters *_Nullable)parameters;
...
/**
 *  线程队列的使用
 *  @param block    block，将数 _Nullable 据库操作放到block里执行可以保证线程安全
 */
 ...
 *  @param block    block，将数 _Nullable 据库操作放到block里执行可以保证线程安全
 */
- (void)inTransaction:(void(^_Nullable)(BOOL * _Nullable rollback))block;
