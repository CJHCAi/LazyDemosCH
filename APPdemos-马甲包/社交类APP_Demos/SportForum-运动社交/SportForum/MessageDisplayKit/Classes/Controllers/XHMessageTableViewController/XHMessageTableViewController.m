...
- (UIView *)headerContainerView {
    if (!_headerContainerView) {
        _headerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), 20)];
...
- (void)initilzer {
    ...
    XHMessageTableView *messageTableView = [[XHMessageTableView alloc] initWithFrame:CGRectMake(0, 0, viewBody.frame.size.width, viewBody.frame.size.height) style:UITableViewStylePlain];
    ...
    [viewBody addSubview:messageTableView];
    [viewBody sendSubviewToBack:messageTableView];
...
- (void)layoutAndAnimateMessageInputTextView:(UITextView *)textView {
    ...
    if (!isShrinking && (self.previousTextViewContentHeight == maxHeight || textView.text.length == 0 || colomNumber == 1)) {
        changeInHeight = 0;
    }
    else {
