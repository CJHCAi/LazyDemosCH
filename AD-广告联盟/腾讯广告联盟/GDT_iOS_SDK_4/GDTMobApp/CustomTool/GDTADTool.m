...
+(GDTMobBannerView *)GDT_setupBannerView{
    ...
    GDTMobBannerView * bannerView = [[GDTMobBannerView alloc] initWithAppId:AppId placementId:bannerID];
...
+(GDTMobInterstitial *)GDT_setupInterstitial{
    GDTMobInterstitial * interstitial = [[GDTMobInterstitial alloc] initWithAppId:AppId placementId:interstitialID];
...
//    }
    ...
    GDTNativeAd * nativeAd = [[GDTNativeAd alloc] initWithAppId:AppId placementId:nativeID];
...
+(GDTSplashAd *)GDT_FullSplashAD:(UIWindow *)window{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
        GDTSplashAd *splash = [[GDTSplashAd alloc] initWithAppId:AppId placementId:splashID];
...
+(GDTSplashAd *)GDT_setupSplashADWithBottomView:(UIView *)bottomView  skipView:(UIView *)skipView{
    ...
    GDTSplashAd * splashAd = [[GDTSplashAd alloc] initWithAppId:AppId placementId:splashID];
...
+(GDTNativeExpressAd *)GDT_setupNativeExpressADWithWidth:(CGFloat)width height:(CGFloat)height Adcount:(NSInteger)adCount{
    GDTNativeExpressAd * nativeExpressAd = [[GDTNativeExpressAd alloc] initWithAppId:AppId placementId:@"5030722621265924" adSize:CGSizeMake(width, height)];
...
+(GDTNativeExpressAd *)GDT_setupNativeVideoExpressADWithSize:(CGSize)size autoPlay:(BOOL)autoPlay videoMute:(BOOL)videoMute adCount:(NSInteger)adCount{
    ...
    GDTNativeExpressAd * nativeExpressAd = [[GDTNativeExpressAd alloc] initWithAppId:AppId placementId:@"1020922903364636" adSize:CGSizeEqualToSize(size, CGSizeZero)?CGSizeMake(ScreenWidth,50):size];
//    nativeExpressAd.delegate = self;
