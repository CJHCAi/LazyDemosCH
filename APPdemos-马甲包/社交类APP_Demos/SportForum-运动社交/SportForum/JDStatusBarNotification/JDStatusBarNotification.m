...
@property (nonatomic, strong) OnClickViewBlock onClickViewBlock;
@end
             ...
             dismissAfter:(NSTimeInterval)timeInterval;
{
    ...
    {
        [AlertManager showAlertText:status];
    }
...
+ (UIView*)showWithStatus:(NSString *)status
                      dismissAfter:(NSTimeInterval)timeInterval
                         styleName:(NSString*)styleName
                       actionBlock:(OnClickViewBlock)onClickBlock
{
    UIView *view = [[self sharedInstance] showWithStatus:status
                                               styleName:styleName actionBlock:onClickBlock];
    view.hidden = YES;
                ...
                actionBlock:(OnClickViewBlock)onClickBlock
{
    ...
    self->_onClickViewBlock = onClickBlock;
                    ...
                    style:(JDStatusBarStyle*)style;
{
    ...
    CGSize lbSize = [textLabel.text boundingRectWithSize:CGSizeMake(CGRectGetWidth(self.topBar.frame) - 20, FLT_MAX)
                                             options:options
                                          attributes:@{NSFontAttributeName:textLabel.font} context:nil].size;
    textLabel.frame = CGRectMake(10, 5, CGRectGetWidth(self.topBar.frame) - 20, lbSize.height + 8);
    
    CGRect rect = self.topBar.frame;
    rect.size = CGSizeMake(CGRectGetWidth(self.topBar.frame), lbSize.height + 16);
    ...
    if (_onClickViewBlock != nil && style == self.userStyles[JDStatusBarStyleDark]) {
        CSButton *btnClick = self.topBar.btnAction;
        btnClick.frame = CGRectMake(0, 0, CGRectGetWidth(self.topBar.frame), CGRectGetHeight(self.topBar.frame));
        btnClick.actionBlock = self->_onClickViewBlock;
    }
...
- (UIWindow *)overlayWindow;
{
    if(_overlayWindow == nil) {
        _overlayWindow = [[CusWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
