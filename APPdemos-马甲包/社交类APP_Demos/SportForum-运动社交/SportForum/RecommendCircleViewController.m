//
//  RecommendCircleViewController.m
//  SportForum
//
...
@interface RecommendCircleViewController ()<UITableViewDelegate, UITableViewDataSource, MWPhotoBrowserDelegate, EGORefreshTableHeaderDelegate, UIScrollViewDelegate, SwipeViewDelegate, SwipeViewDataSource, XHMessageInputViewDelegate, UIImagePickerControllerDelegate,ZYQAssetPickerControllerDelegate, UINavigationControllerDelegate,UIAlertViewDelegate, UITextFieldDelegate>

@end

@implementation RecommendCircleViewController
{
    UIView *m_barView;
...
- (void)viewDidLoad {
    ...
    [self generateCommonViewInParent:self.view Title:_bAttentionCircle ? @"关注动态" : @"悦动吧" IsNeedBackBtn:YES ActionBlock:^(void)
    {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MobClick beginLogPageView:@"关注动态/悦动吧 - RecommendCircleViewController"];
    [[ApplicationContext sharedInstance]setRegUserPath:@"关注动态/悦动吧 - RecommendCircleViewController"];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [MobClick endLogPageView:@"关注动态/悦动吧 - RecommendCircleViewController"];
...
-(void)dealloc
{
    NSLog(@"RecommendCircleViewController dealloc called!");
...
-(UIView*)generateRewardView:(NSInteger)nCoinValue
{
    ...
    lbTips.text = [NSString stringWithFormat:@"亲，您当前的金币可支配额为%lu个, 默认打赏数目为1个金币。", nCoinValue];
...
- (void)updateUI:(NSUInteger)nUpdateItem {
    ...
    label.text = [NSString stringWithFormat:_bAttentionCircle ? @"更新了%lu条关注动态" : @"更新了%lu条博文",nUpdateItem];
    [label sizeToFit];
...

{
    ...
    [[SportForumAPI sharedInstance]articleTimeLinesByFirPagId:strFirstrPageId
                                                   LastPageId:strLastPageId
                                                PageItemCount:10
                                                   ArticleTag:e_article_log
                                                     IsCircle:_bAttentionCircle
                                                FinishedBlock:^(int errorCode, ArticlesInfo *articlesInfo)
     {
         ...
         if (errorCode == 0 && [articlesInfo.articles_without_content.data count] > 0)
         {
             ...
             else if(strFirstrPageId.length > 0 && strLastPageId.length == 0)
             {
                 ...
                 NSRange range = NSMakeRange(0, [articlesInfo.articles_without_content.data count]);
                 NSIndexSet *indexSet = [NSIndexSet indexSetWithIndexesInRange:range];
                 [strongSelf->_arrAttentions insertObjects:articlesInfo.articles_without_content.data atIndexes:indexSet];
             }

             if (bLoadLatest && _bAttentionCircle) {
                 ArticlesObject *articleObject = articlesInfo.articles_without_content.data[0];
                 NSMutableDictionary *articleDict = [NSMutableDictionary dictionaryWithObjectsAndKeys:articleObject.article_id, @"ArticleId", nil];
                 [[ApplicationContext sharedInstance] saveObject:articleDict byKey:@"AttentionCircleId"];
        ...
        if (strongSelf->_textViewInputViewType == XHInputViewTypeText) {
            [UIView animateWithDuration:duration
                             ...
                             animations:^{
                                 ...
                                 CGRect inputViewFrame = strongSelf->m_viewReply.frame;
                                 CGFloat inputViewFrameY = (showKeyborad ? MIN(keyboardY - inputViewFrame.size.height, inputViewFrame.origin.y) : keyboardY);
...
-(void)showPicSelect {
    ...
    if (_customMenuViewController == nil) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.articlesObject != articlesObject) {
        ...
        cell.tutorBlock = ^void()
        {
            ...
            chatMessageTableViewController.userId = articlesObject.authorInfo.userid;
            chatMessageTableViewController.useProImage = articlesObject.authorInfo.profile_image;
            chatMessageTableViewController.useNickName = articlesObject.authorInfo.nikename;
