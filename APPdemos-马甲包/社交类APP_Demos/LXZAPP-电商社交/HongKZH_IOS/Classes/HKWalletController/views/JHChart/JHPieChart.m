...
- (void)judgeWhitchOneIsNowAngle:(CGFloat )angel andShowPoint:(CGPoint)p{
    ...
    for (NSInteger i = 0; i<_countPreAngeleArr.count-1; i++) {
        ...
        if ([_countPreAngeleArr[i+1] floatValue]>=angel) {
            ...
            DLog(@"%@",NSStringFromCGPoint(centerPoint));
            ...
            [UIView animateWithDuration:0.3 animations:^{
                if (self.saveIndex==i) {
                    ...
                    if (saveItems.center.x==self.frame.size.width/2) {
                        ...
                        [self.showInfoView updateFrameTo:CGRectMake(p.x, p.y, self.showInfoView.frame.size.width, self.showInfoView.frame.size.height) andBGColor:colors[i%colors.count] andShowContentString:[NSString stringWithFormat:@"%@ 数量:% 3ld 占比:%.1f%c",self.descArr[i],[self.valueArr[i] integerValue],present,'%']];
