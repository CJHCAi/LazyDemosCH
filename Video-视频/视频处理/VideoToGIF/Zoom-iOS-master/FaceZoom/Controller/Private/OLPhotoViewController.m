...
@property (nonatomic, strong) NSMutableArray *selectedImagesInFuturePages; // selected images that don't yet occur in collectionView.indexPathsForSelectedItems as the user needs to load more instagram pages first
...
-(void)showTheThing:(UITapGestureRecognizer *)thisTap{
    /*
    thisInd = [[UIActivityIndicatorView alloc]initWithFrame:[UIScreen mainScreen].bounds];
    thisInd.activityIndicatorViewStyle = UIActivityIndicatorViewStyleWhiteLarge;
    thisInd.layer.opacity = 0.65;
    [thisInd startAnimating];
    [self.view insertSubview:thisInd atIndex:99];
     */
