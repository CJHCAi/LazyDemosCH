...
+ (void)showInfoWithStatus:(NSString*)status hidewithInterVal:(float)interval{
    [self setDefaultMaskType:SVProgressHUDMaskTypeClear];
    [self showImage:[self sharedView].infoImage status:status hideInterVal:interval];
}

+ (void)showInfoWithStatus:(NSString*)status hidewithInterVal:(float)interval maskType:(SVProgressHUDMaskType)maskType {
    SVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
    [self showInfoWithStatus:status hidewithInterVal:interval];
    [self setDefaultMaskType:existingMaskType];
}

+ (void)showSuccessWithStatus:(NSString*)status hidewithInterVal:(float)interval{
    [self setDefaultStyle:SVProgressHUDStyleLight];
    [self setDefaultMaskType:SVProgressHUDMaskTypeClear];
    [self showImage:[self sharedView].successImage status:status hideInterVal:interval];
}

+ (void)showSuccessWithStatus:(NSString*)status hidewithInterVal:(float)interval maskType:(SVProgressHUDMaskType)maskType {
    SVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
    [self showSuccessWithStatus:status hidewithInterVal:interval];
    [self setDefaultMaskType:existingMaskType];
}

+ (void)showErrorWithStatus:(NSString*)status hideInterVal:(float)interval{
    [self setDefaultMaskType:SVProgressHUDMaskTypeClear];
    [self showImage:[self sharedView].errorImage status:status hideInterVal:interval];
}

+ (void)showErrorWithStatus:(NSString*)status hideInterVal:(float)interval maskType:(SVProgressHUDMaskType)maskType {
    SVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
    [self showErrorWithStatus:status hideInterVal:interval];
    [self setDefaultMaskType:existingMaskType];
}

+ (void)showImage:(UIImage*)image status:(NSString*)status  hideInterVal:(float)interval{
    //NSTimeInterval displayInterval = [self displayDurationForString:status];
    NSTimeInterval displayInterval = interval;
    [[self sharedView] showImage:image status:status duration:displayInterval];
}

+ (void)showImage:(UIImage*)image status:(NSString*)status hideInterVal:(float)interval maskType:(SVProgressHUDMaskType)maskType {
    SVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
    [self showImage:image status:status hideInterVal:interval maskType:maskType];
    [self setDefaultMaskType:existingMaskType];
}
