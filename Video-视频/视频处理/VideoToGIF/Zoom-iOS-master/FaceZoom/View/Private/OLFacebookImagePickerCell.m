...
- (void)bind:(OLFacebookImage *)media {
    ...
    self.fullURL = [media bestURLForSize:CGSizeMake(1000, 1000)];
    [self.imageView setAndFadeInFacebookImageWithURL:[media bestURLForSize:CGSizeMake(150, 150)]]; // opted for slightly larger than thumb url. Might be better for performance just to go with thumb.
}
