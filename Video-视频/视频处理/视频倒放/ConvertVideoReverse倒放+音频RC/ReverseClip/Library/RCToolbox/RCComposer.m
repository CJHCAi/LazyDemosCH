...
//    CMTimeRange editRange = CMTimeRangeMake(inTime, duration);
    ...
    [_compositionVideoTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, urlAsset.duration) ofTrack:clipVideoTrack atTime:self.composition.duration error:&editError];
    
    if (!editError) {
        NSLog(@"-- %f",CMTimeGetSeconds (self.composition.duration));
