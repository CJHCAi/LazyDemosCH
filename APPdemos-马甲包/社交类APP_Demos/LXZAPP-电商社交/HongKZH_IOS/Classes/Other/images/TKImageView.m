    ...
    switch (panGesture.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            switch (midLineView.type) {
                case TKMidLineTypeTop: {
                    CGFloat minWidth = kScreenWidth;
                    CGFloat maxWidth = kScreenWidth;
                    CGFloat minHeight = 226;
                    CGFloat maxHeight = 458;
                    CGFloat willHeight = MIN(MAX(minHeight, CGRectGetHeight(_cropAreaOriFrame) - translation.y), maxHeight);
                    if (willHeight>maxHeight) {
                        willHeight = maxHeight;
                    }
                    CGFloat deltaY = willHeight - CGRectGetHeight(_cropAreaOriFrame);
                    _cropAreaView.frame = CGRectMake(CGRectGetMinX(_cropAreaOriFrame), CGRectGetMinY(_cropAreaOriFrame) - deltaY, maxWidth, willHeight);
                    break;
                }
                case TKMidLineTypeBottom: {
                    CGFloat minWidth = kScreenWidth;
                    ...
                    _cropAreaView.frame = CGRectMake(CGRectGetMinX(_cropAreaOriFrame), CGRectGetMinY(_cropAreaOriFrame), maxWidth, willHeight);
                    break;
                }
                case TKMidLineTypeLeft: {
                    CGFloat minWidth = kScreenWidth;
...
//                    CGFloat willWidth = MIN(MAX(minWidth, CGRectGetWidth(_cropAreaOriFrame) - translation.x), maxWidth);
                    CGFloat willWidth = maxWidth;
                    CGFloat deltaX = willWidth - CGRectGetWidth(_cropAreaOriFrame);
                    _cropAreaView.frame = CGRectMake(CGRectGetMinX(_cropAreaOriFrame) - deltaX, CGRectGetMinY(_cropAreaOriFrame), willWidth, CGRectGetHeight(_cropAreaOriFrame)>maxHeight?maxHeight:CGRectGetHeight(_cropAreaOriFrame));
                    break;
                }
                case TKMidLineTypeRight: {
                    CGFloat minWidth = kScreenWidth;
...
//                    CGFloat willWidth = MIN(MAX(minWidth, CGRectGetWidth(_cropAreaOriFrame) + translation.x), maxWidth);
                    CGFloat willWidth = maxWidth;
                    _cropAreaView.frame = CGRectMake(CGRectGetMinX(_cropAreaOriFrame), CGRectGetMinY(_cropAreaOriFrame), willWidth, CGRectGetHeight(_cropAreaOriFrame)>maxHeight?maxHeight:CGRectGetHeight(_cropAreaOriFrame));
                    break;
                }
                default:
                    break;
...
- (void)resetMidLines {
    ...
    CGFloat lineMargin = _cropAreaMidLineHeight / 2.0 - _cropAreaBorderLineWidth;
     CGFloat h = MID_LINE_INTERACT_WIDTH - ( - MID_LINE_INTERACT_HEIGHT / 2.0 - lineMargin);
    if (h) {
...
- (void)resetCropAreaByAspectRatio {
    ...
    _cropAreaView.frame = CGRectMake((WIDTH(_imageView) - width) / 2.0, (HEIGHT(_imageView) - height) / 2.0, width, 458);
    [self resetCornersOnCropAreaFrameChanged];
    [self resetCropTransparentArea];
...
- (void)resetCropAreaByScaleFactor: (CGFloat)scaleFactor {
    ...
    CGFloat heightMax = 229;
    CGFloat heightMin = 113;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    ...
    if([object isEqual: _cropAreaView]) {
        if ([keyPath isEqualToString:@"frame"]) {
            if (_cropAreaView.frame.size.width>kScreenWidth) {
                _cropAreaView.width = kScreenWidth;
            }
            if (_cropAreaView.frame.size.height>458) {
                _cropAreaView.height = 458;
            }
            if (_cropAreaView.frame.size.height<226) {
                _cropAreaView.height = 226;
            }
        }
        if(_showMidLines){
            [self resetMidLines];
