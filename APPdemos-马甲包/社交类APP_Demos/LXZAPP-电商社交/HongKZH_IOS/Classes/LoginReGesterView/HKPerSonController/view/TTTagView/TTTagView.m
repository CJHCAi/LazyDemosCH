...
//-(instancetype)initWithframe:(CGRect)frame andType:(TTTagView_Type)type {
//    self =[super initWithFrame:frame];
//    if (self) {
//        self.type =type;
//         [self commonInit];
...
- (void)commonInit
{
   ...
   // _type = TTTagView_Type_Display;
     ...
     *  标签字体大小
     */
    _fontTag = [UIFont systemFontOfSize:13];
    /**
     *  输入标签的字体大小
     */
    _fontInput = [UIFont systemFontOfSize:13];
...
//    _selBorderColor = kColorRGB(0xffae00);
    ...
    if (self.type ==TTTagView_Type_Edit) {
        // 默认的标签
        UITextField* tf = [[TTTextField alloc] initWithFrame:CGRectMake(-10, 10, _tagWidht, _tagHeight)];
        tf.autocorrectionType = UITextAutocorrectionTypeNo;
        [tf addTarget:self action:@selector(textFieldDidFinishChange:)forControlEvents:UIControlEventEditingChanged];
        tf.delegate = self;
        tf.backgroundColor =RGB(237,237,237);
        NSMutableParagraphStyle *style = [[NSMutableParagraphStyle alloc] init];
        style.alignment = NSTextAlignmentCenter;
        NSAttributedString *attri = [[NSAttributedString alloc] initWithString:@"+" attributes:@{NSForegroundColorAttributeName:RGB(132,132,132),NSFontAttributeName:[UIFont systemFontOfSize:13], NSParagraphStyleAttributeName:style}];
...
-(void)layoutTagviews {
    ...
    if (_type==TTTagView_Type_Edit) {
        ...
        _tfInput.layer.cornerRadius = _tfInput.frame.size.height * 0.1f;
       // _tfInput.layer.borderWidth=1;
        
         CGFloat w = _tfInput.frame.size.height *0.1f;
        {
            CGRect frame=_tfInput.frame;
            frame.size.width = [_tfInput.text sizeWithAttributes:@{NSFontAttributeName:_fontInput}].width + w* 2.0f + _textPaddingSize.width * 2.0;
...
- (TTCheckBoxButton *)tagButtonWithTag:(NSString *)tag {
    ...
    if (self.type ==TTTagView_Type_Edit) {
        [tagBtn setImage:[UIImage imageNamed:@"delete"] forState:UIControlStateNormal];
        [tagBtn setImagePosition:1 spacing:3];
    }
    CGRect btnFrame = CGRectMake(0, 0, 0, 0);
    btnFrame.size.height = _tagHeight;
    tagBtn.layer.cornerRadius = btnFrame.size.height * 0.1f;
    CGFloat w = btnFrame.size.height *0.5f;
    
    btnFrame.size.width = [tagBtn.titleLabel.text sizeWithAttributes:@{NSFontAttributeName:_fontTag}].width + (w * 2.0f);
