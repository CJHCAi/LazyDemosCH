...
-(void)reloadData {
    ...
    if(numberOfPhotos > 0) {
        ...
        for (NSUInteger index = 0; index < numberOfPhotos; index++) {
            CGSize sizeImg = CGSizeMake(self.frame.size.width - self.borderWidth*2, self.frame.size.height - self.borderWidth*2);
            NSString *imageUrl = [self.dataSource photoStackView:self photoForIndex:index];
            UIImageView *photoImageView     = [[UIImageView alloc] initWithFrame:(CGRect){CGPointZero, sizeImg}];
            photoImageView.contentMode = UIViewContentModeScaleAspectFill;
            photoImageView.layer.masksToBounds = YES;
            [photoImageView sd_setImageWithURL:[NSURL URLWithString:imageUrl]
