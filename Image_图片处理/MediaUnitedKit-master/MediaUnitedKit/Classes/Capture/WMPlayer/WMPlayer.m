...
-(void)creatFF_View
{
    ...
    self.FF_View.backgroundColor = RGBColor(0, 0, 0, 0.7);
    self.FF_View.hidden = YES;
    self.FF_View.layer.cornerRadius = 4.0;
...
-(void)initWMPlayer
{
    ...
    self.topView.image = WMPlayerImage(@"vmplayer_top");
    ...
    self.bottomView.image = WMPlayerImage(@"vmplayer_bottom");
    ...
    [self.playOrPauseBtn setImage:WMPlayerImage(@"vmplayer_pause") forState:UIControlStateNormal];
    [self.playOrPauseBtn setImage:WMPlayerImage(@"vmplayer_play") forState:UIControlStateSelected];
    ...
    //_playBtn
    UIImage *image = WMPlayerImage(@"vmplayer_play_m");
    self.playBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, image.size.width, image.size.height)];
    ...
    [self.progressSlider setThumbImage:WMPlayerImage(@"vmplayer_dot") forState:UIControlStateNormal];
...
//    self.fullScreenBtn.backgroundColor = [UIColor redColor];
    ...
    [self.fullScreenBtn setImage:WMPlayerImage(@"vmplayer_fullscreen") forState:UIControlStateNormal];
    [self.fullScreenBtn setImage:WMPlayerImage(@"vmplayer_fullscreen_no") forState:UIControlStateSelected];
...
//    _closeBtn.backgroundColor = [UIColor redColor];
    ...
    [self.topView bringSubviewToFront:_closeBtn];
...
- (void)appDidEnterBackground:(NSNotification*)note
{
    if (self.playOrPauseBtn.isSelected==NO) {//如果是播放中，则继续播放
        ...
        [self.player play];
        DLog(@"22222 %s WMPlayerStatePlaying",__FUNCTION__);
        self.state = WMPlayerStatePlaying;
    }else{
        DLog(@"%s WMPlayerStateStopped",__FUNCTION__);
...
- (void)appWillEnterForeground:(NSNotification*)note
{
    if (self.playOrPauseBtn.isSelected==NO) {//如果是播放中，则继续播放
        ...
        [self.player play];
        self.state = WMPlayerStatePlaying;
        DLog(@"3333333%s WMPlayerStatePlaying",__FUNCTION__);
    } else {
        DLog(@"%s WMPlayerStateStopped",__FUNCTION__);
        self.state = WMPlayerStateStopped;
    }
...
- (void)fullScreenAction:(UIButton *)sender
{
    ...
    //小屏时隐藏关闭和标题
    self.closeBtn.hidden = !self.isFullscreen;
...
- (void)play
{
    [self setIsHiddenControlView:NO];
...
- (void)setURLString:(NSString *)URLString{
    ...
    if (self.closeBtnStyle==CloseBtnStylePop) {
        [_closeBtn setImage:WMPlayerImage(@"vmplayer_back") forState:UIControlStateNormal];
        [_closeBtn setImage:WMPlayerImage(@"vmplayer_back") forState:UIControlStateSelected];
        
    }else{
        [_closeBtn setImage:WMPlayerImage(@"vmplayer_close") forState:UIControlStateNormal];
        [_closeBtn setImage:WMPlayerImage(@"vmplayer_close") forState:UIControlStateSelected];
    }
}
...
- (void)setIsHiddenControlView:(BOOL)isHiddenControlView
{
    ...
    if (context == PlayViewStatusObservationContext)
    {
        if ([keyPath isEqualToString:@"status"]) {
            ...
            switch (status)
            {
                ...
                {
                    [self.loadingProgress setProgress:0.0 animated:NO];
                    DLog(@"%s WMPlayerStateBuffering",__FUNCTION__);
                ...
                case AVPlayerStatusFailed:
                {
                    ...
                    DLog(@"%s WMPlayerStateFailed",__FUNCTION__);
            ...
            if (self.currentItem.playbackLikelyToKeepUp && self.state == WMPlayerStateBuffering){
                DLog(@"55555%s WMPlayerStatePlaying",__FUNCTION__);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([[(UITouch *)touches.anyObject view] isEqual:self.closeBtn] ) {
        return;
    }
...
- (void)timeValueChangingWithValue:(float)value
{
    if (value > _touchBeginValue) {
        self.FF_View.sheetStateImageView.image = WMPlayerImage(@"vmplayer_pg_ forward");
    } else if(value < _touchBeginValue){
        self.FF_View.sheetStateImageView.image =WMPlayerImage(@"vmplayer_pg_back");
    }
