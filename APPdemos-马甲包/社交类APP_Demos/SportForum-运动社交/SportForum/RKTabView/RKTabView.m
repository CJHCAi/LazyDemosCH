...
- (UIColor *)titlesFontColor {
    if (!_titlesFontColor) {
        _titlesFontColor = [UIColor blackColor];
    }
    return _titlesFontColor;
}

- (UIColor *)titlesEnableFontColor {
    if (!_titlesEnableFontColor) {
        _titlesEnableFontColor = [UIColor whiteColor];
    }
    return _titlesEnableFontColor;
}
...
- (void)setTabContent:(UIControl *)tab withTabItem:(RKTabItem *)tabItem {
    ...
    if (tabItem.titleString.length != 0) {
        ...
        if (tabItem.tabState == TabStateEnabled) {
            if (tabItem.titleEnableFontColor) {
                textColor = tabItem.titleEnableFontColor;
            } else if (!tabItem.titleEnableFontColor && self.titlesEnableFontColor) {
                textColor = self.titlesEnableFontColor;
            }
        }
        else
        {
    ...
    rect = ((UIView *)interfaceElement).frame;
    
    //Subviews frames
    if (titleLabel) {
        //CGFloat wholeGapHeight = tab.bounds.size.height - ((UIView *)interfaceElement).bounds.size.height - titleSize.height;
        titleLabel.frame = CGRectMake(tab.bounds.size.width/2 - titleSize.width/2, ((UIView *)interfaceElement).bounds.size.height - titleSize.height - 5, titleSize.width, titleSize.height+2);
        rect = titleLabel.frame;
        
        ((UIView *)interfaceElement).frame = CGRectMake(tab.bounds.size.width/2 - ((UIView *)interfaceElement).bounds.size.width/2, 0, ((UIView *)interfaceElement).bounds.size.width, ((UIView *)interfaceElement).bounds.size.height);
        rect = ((UIView *)interfaceElement).frame;
