    ...
    for (CIFeature *f in faceArray){
        ...
        if ([f.type isEqualToString:CIFeatureTypeFace]) {
            ...
            if(faceFeature.hasMouthPosition){
                ...
                CGFloat x=faceFeature.mouthPosition.x;
                CGFloat y=faceFeature.mouthPosition.y;
                CGRect MouthRect=CGRectMake(x-10/2,y-10/2, 10, 10);
                
                //获取人脸的frame
                CGRect mouthBounds = CGRectApplyAffineTransform(MouthRect, transform);
                mouthBounds=CGRectApplyAffineTransform(mou,scaleTransform);
                rightEyeBounds.origin.x += offsetX;
                rightEyeBounds.origin.y += offsetY;
