...
- (void)handlerTagButtonEvent:(TTTagViewCheckBoxButton*)sender
{
    if (self.canDeselect) {
        for (UIView * view  in self.subviews) {
            if ([view isKindOfClass:[TTTagViewCheckBoxButton class]]) {
                TTTagViewCheckBoxButton *b =(TTTagViewCheckBoxButton *)view;
                b.selected = NO;
            }
        }
        sender.selected =YES;
        if ([self.delegate respondsToSelector:@selector(buttonClick:and:)]) {
            [self.delegate buttonClick:sender.titleLabel.text and:sender.selected];
        }
    }else {
