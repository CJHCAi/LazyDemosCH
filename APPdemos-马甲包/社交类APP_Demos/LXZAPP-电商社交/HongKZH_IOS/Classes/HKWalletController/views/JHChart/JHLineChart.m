...
-(void)addPieForeView
{
    if (_pieForeView == nil) {
        ...
        _pieForeView.select = ^(CGFloat angle,CGPoint p){
            @strongify(self)
            DLog(@"选中的角度: %f 点击的点: %@",angle,NSStringFromCGPoint(p)
                  );
            [self judgeWhitchOneWithPoint:p];
        };
...
-(void)addLineItem
{
    if (_lineItemView == nil) {
        _lineItemView = [[JHLineItemView alloc] initWithFrame:CGRectMake(0, 0, 70, 30)];
        _lineItemView.alpha = 0.0;
...
{
    DLog(@"x 轴每个刻度间距 %f",_perXLen);
    DLog(@"y 轴每个刻度间距 %f",_perXLen);
    DLog(@"所设置的内容的偏移量 %@",NSStringFromUIEdgeInsets(self.contentInsets));
    ...
    if (p.y > a) { // 点击在图片范围之内
        DLog(@"所选中的月份:  %@",self.xLineDataArr[index]);
        DLog(@"所点中的y 值:  %f",a);
        // 绘制用户数据
