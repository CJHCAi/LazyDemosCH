...
@interface PYSearchViewController () <UITableViewDelegate, UITableViewDataSource,UITextFieldDelegate>
...
+ (PYSearchViewController *)searchViewControllerWithHotSearches:(NSArray<NSString *> *)hotSearches searchBarPlaceholder:(NSString *)placeholder
{
    ...
    searchVC.textField.placeholder = placeholder;
    return searchVC;
}
...
- (PYSearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        searchSuggestionVC.didSelectCellBlock = ^(UITableViewCell *didSelectCell) {
            // 设置搜索信息
            _weakSelf.textField.text = didSelectCell.textLabel.text;
            // 点击搜索
            [_weakSelf textFieldShouldReturn:_weakSelf.textField];
        };
...
- (void)setup{
    ...
    UIBarButtonItem * backItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"black_return"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStyleDone target:self action:@selector(backItemClicked)];
    ...
    // 创建搜索框
    _textField = [[CHTextField alloc] initWithFrame:CGRectMake(0, 0, 300, 30)];
    _textField.py_width -= PYMargin * 1.5;
    _textField.placeholder = PYSearchPlaceholderText;
...
-(BOOL)textFieldShouldReturn:(CHTextField *)textField{
    [textField resignFirstResponder];
    [self.searchHistories removeObject:textField.text];
    [self.searchHistories insertObject:textField.text atIndex:0];
    // 刷新数据
    ...
    // 如果代理实现了代理方法则调用代理方法
    if ([self.delegate respondsToSelector:@selector(searchViewController:didSearchWithTextField:searchText:)]) {
        [self.delegate searchViewController:self didSearchWithTextField:textField searchText:textField.text];
        return YES;
    }
    // 如果有block则调用
    if (self.didSearchBlock) self.didSearchBlock(self, textField.text);

    return YES;
}

-(void)textFieldTextChanged:(CHTextField *)textField{
    // 根据输入文本显示建议搜索条件
    self.searchSuggestionVC.view.hidden = self.searchSuggestionHidden || !textField.text.length;
    // 放在最上层
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    if (self.keyboardshowing) [self.textField resignFirstResponder];
}
