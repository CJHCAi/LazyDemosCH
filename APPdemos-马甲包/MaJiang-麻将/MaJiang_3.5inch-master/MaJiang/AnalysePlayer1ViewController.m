//
//  AnalysePlayer1ViewController.m
//  MaJiang
//
...
#import "AnalysePlayer1ViewController.h"
#import "PNChart.h"
...
@interface AnalysePlayer1ViewController ()
{
...
@implementation AnalysePlayer1ViewController
...
- (void)viewDidLoad
{
    ...
    if (sqlite3_open(dbpath, &database) == SQLITE_OK)
    {
        ...
        for (int i=0; i<DBgameid; i++) {
            NSString *sqlQuery2 = [NSString stringWithFormat:@"SELECT SUM(PLAYER1) FROM POINTS WHERE GAMEID = %d", i + 1];
        ...
        if (sqlite3_prepare_v2(database, [sqlQuery3 UTF8String], -1, &statement, nil) == SQLITE_OK) {
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ...
                if ([nsfactor3 isEqualToString:@"对家"]) {
                    //NSLog(@"%@", nsfactor3);
                    if ([nsfactor2 isEqualToString:@"接炮"]) {
                        dianpao++;
                    }
        ...
        if (tmpLineData.count == 0) {
            linedata = [[NSArray alloc] initWithObjects:[NSString stringWithFormat:@"0"], nil];
        } else
        {
            NSLog(@"not nil");
