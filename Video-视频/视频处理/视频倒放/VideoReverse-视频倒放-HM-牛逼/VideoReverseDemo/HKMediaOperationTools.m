...
+ (NSURL *)assetByReversingAsset:(AVAsset *)asset videoComposition:(AVMutableVideoComposition *)videoComposition duration:(CMTime)duration outputURL:(NSURL *)outputURL progressHandle:(HKProgressHandle)progressHandle cancle:(BOOL *)cancle {
    if (*(cancle)) {
        return nil;
    }
    NSError *error;
    ...
    //初始化一个输入适配器，以便我们可以添加PixelBuffer
    ...
    for (NSInteger i = tracks.count -1; i <= tracks.count; i --) {
        ...
        for(NSInteger j = 0; j < countOfFrames; j++) {
            // 获取框架的演示时间 Get the presentation time for the frame
