...
- (void)moveToControllerAtIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    //有修改
    [_contentView setContentOffset:CGPointMake(index * CGRectGetWidth(_contentView.frame), _contentView.contentOffset.y) animated:NO];
}
...
- (void)reSizeContentView
{
    ...
    _contentView.contentOffset = CGPointMake(_curIndex*CGRectGetWidth(_contentView.frame), _contentView.contentOffset.y);
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    if (scrollView == _contentView) {
        ...
        if(self.willDraggingBlock){self.willDraggingBlock();}
...
{
    if([self.delegate respondsToSelector:@selector(pagerControllerDidEndDecelerating:)]){
        [self.delegate pagerControllerDidEndDecelerating:self];
    }
}
