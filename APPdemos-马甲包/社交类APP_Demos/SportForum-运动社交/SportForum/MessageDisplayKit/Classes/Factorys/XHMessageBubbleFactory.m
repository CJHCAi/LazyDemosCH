...
+ (UIImage *)bubbleImageViewForType:(XHBubbleMessageType)type
    ...
    switch (type) {
        case XHBubbleMessageTypeSending:
            // 发送
            messageTypeString = @"message-1";//[messageTypeString stringByAppendingString:@"_Sending"];
            break;
        case XHBubbleMessageTypeReceiving:
            // 接收
            messageTypeString = @"message-2";//[messageTypeString stringByAppendingString:@"_Receiving"];
            break;
        default:
            break;
    }
    ...
    UIEdgeInsets bubbleImageEdgeInsets = UIEdgeInsetsMake(floorf(bublleImage.size.height / 2) + 5 , floorf(bublleImage.size.width / 2) + 5, floorf(bublleImage.size.height / 2) + 5, floorf(bublleImage.size.width / 2) + 5);//[self bubbleImageEdgeInsetsWithStyle:style];
