...
+(UIImage *)imageNamedWithWebP:(NSString *)fileWebpName
{
    NSString *normalImg = @"";
    
    if(IS_RETINA)
    {
        normalImg = [[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"%@@2x", fileWebpName] ofType:@"webp"];
        
        if (normalImg.length == 0) {
            normalImg = [[NSBundle mainBundle] pathForResource:fileWebpName ofType:@"webp"];
        }
    }
    else
    {
    ...
    UIImage *image = [self imageWithWebP:normalImg];
    return image;
}

+(UIImage *)imageNamedWithWebPPath:(NSString *)fileWebpPath
{
    UIImage *image = [self imageWithWebP:fileWebpPath];
    return image;
}
