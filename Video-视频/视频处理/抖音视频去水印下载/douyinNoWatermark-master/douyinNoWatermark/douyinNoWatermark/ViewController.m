...
- (void)getUrlFromSrcHtmlWithSrcUrl:(NSString *)urlstr finish:(void(^)(NSString *url))block {
    urlstr = self.textField.text;//@"https://v.douyin.com/G6HEWY/";
...
- (void)getFakeTrueUrlWithString:(NSString *)string finish:(void(^)(NSString *url))block{
    ...
    [pattern enumerateMatchesInString:string options:kNilOptions range:NSMakeRange(0, string.length) usingBlock:^(NSTextCheckingResult * _Nullable result, NSMatchingFlags flags, BOOL * _Nonnull stop) {
        NSLog(@"获取无水印的视频URL");
        NSString *dst = [string substringWithRange:NSMakeRange(result.range.location + 11, result.range.length - 12)];
        NSLog(@"FakeTrueUrl:%@", dst);
        if (block) {
            block(dst);
        }
...
- (IBAction)copyVideoUrlButton:(UIButton *)sender {
    self.videoUrl = @"大宋皇帝的一个心软，成就了千古一帝武则天#武媚娘 #武则天  https://v.douyin.com/JXvLa7a/ 复制此链接，打开抖音搜索，直接观看视频！";
    if (self.videoUrl) {
