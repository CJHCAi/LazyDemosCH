...
+(void)updataFileWithURL:(NSString *)url formDataArray:(NSArray<HKUpdataFromDataModel*>*)formArray parameters:(NSDictionary*)parameters
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:[Host stringByAppendingString:url] parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (int i = 0; i< formArray.count; i++) {
            HKUpdataFromDataModel*dataM = formArray[i];
            if (dataM.type == 0) {
                //文字
               
                NSData *priceData = [dataM.vaule dataUsingEncoding:NSUTF8StringEncoding];
                [formData appendPartWithFormData:priceData name:dataM.name];

            }else if(dataM.type == 1){
               //图片
                NSData *imageData = UIImageJPEGRepresentation(dataM.image, 0.5);
                [formData appendPartWithFileData:imageData name:dataM.name fileName:[NSString stringWithFormat:@"%@%d.%@",dataM.vaule,i,dataM.mimeType ? dataM.mimeType : @"jpeg"] mimeType:[NSString stringWithFormat:@"image/%@",dataM.mimeType ? dataM.mimeType : @"jpeg"]];
            }else{
                [formData appendPartWithFileURL:[NSURL fileURLWithPath:dataM.vaule] name:dataM.name error:&error];
            }
        }
                 ...
                 callback:(HJHttpRequest)callback
{
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        //添加图片,可能有多个参数对应多个图片
        [images enumerateObjectsUsingBlock:^(NSMutableArray<HK_UploadImagesModel *> * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj enumerateObjectsUsingBlock:^(HK_UploadImagesModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                NSData *imageData = UIImageJPEGRepresentation(obj.image, 0.5);
                [formData appendPartWithFileData:imageData name:obj.uploadKey fileName:obj.fileName mimeType:@"image/jpeg"];
            }];
        }];
    } progress:^(NSProgress * _Nonnull uploadProgress) {
        //上传进度
...
{
    [HK_BaseRequest getParameterWithDict:parameters];
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩-添加-上传图片
...
+ (void)uploadEditImageURL:(NSString *)url
        ...
        specifications:(NSArray <SkusModel *> *)specifications
              progress:(HJHttpProgress)progress
              callback:(HJHttpRequest)callback
{
    [HK_BaseRequest getParameterWithDict:parameters];
    NSURLSessionTask *sessionTask = [_sessionManager POST:[Host stringByAppendingString:url] parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        //添加规格
        [specifications enumerateObjectsUsingBlock:^(SkusModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *specifications = obj.model;
            NSData *specificationsData = [specifications dataUsingEncoding:NSUTF8StringEncoding];
            [formData appendPartWithFormData:specificationsData name:@"model"];
            
            //price
            NSString *price = [NSString stringWithFormat:@"%ld",obj.price];
            NSData *priceData = [price dataUsingEncoding:NSUTF8StringEncoding];
            [formData appendPartWithFormData:priceData name:@"price"];
            
            //num
            NSString *num = [NSString stringWithFormat:@"%ld",obj.num];
            NSData *numData = [num dataUsingEncoding:NSUTF8StringEncoding];
            [formData appendPartWithFormData:numData name:@"num"];
            NSString *skuId = obj.skuId;
            NSData *skuIdData = [skuId dataUsingEncoding:NSUTF8StringEncoding];
            [formData appendPartWithFormData:skuIdData name:@"skuId"];
        }];
              ...
              callback:(HJHttpRequest)callback
{
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        //添加规格
        [specifications enumerateObjectsUsingBlock:^(HK_ProductSpecifications * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *specifications = obj.specifications;
            NSData *specificationsData = [specifications dataUsingEncoding:NSUTF8StringEncoding];
            [formData appendPartWithFormData:specificationsData name:@"model"];
...
+ (void)postWithNewURL:(NSString *)url
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:[Host stringByAppendingString:url] parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        [dataArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *value = [obj allValues].lastObject;
            
            NSData *valueData = [value dataUsingEncoding:NSUTF8StringEncoding];
            [formData appendPartWithFormData:valueData name:[obj allKeys].lastObject];
        }];
    } progress:^(NSProgress * _Nonnull uploadProgress) {
        //上传进度
...
+ (void)uploadImageURL:(NSString *)url
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //压缩-添加-上传图片
        [images enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, HK_UploadImagesModel * _Nonnull obj, BOOL * _Nonnull stop) {
            
            NSData *imageData = UIImageJPEGRepresentation(obj.image, 0.5);
            [formData appendPartWithFileData:imageData name:obj.uploadKey fileName:obj.fileName mimeType:[NSString stringWithFormat:@"image/%@",mimeType ? mimeType : @"jpeg"]];
        }];
